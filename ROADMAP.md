...
- structured local deck-level presentation-structure ideation that generates candidate outlines plus per-slide plan changes from saved deck context and can apply one back to persisted deck metadata while promoting retitles into slide files
...
Not implemented yet:
- repo-aware deck-level workflows beyond title promotion and metadata-backed structure planning

## Next Focus

The next practical slice should deepen the deck-level workflow surface now that title promotion is in place:

1. extend deck-level structure planning from title promotion into actual deck-composition changes such as reordering, inserting, or removing slides with explicit apply gating
...
- deck-level presentation-structure ideation through both the browser UI and the assistant, with safe apply back to the saved outline, per-slide structure metadata, and promoted slide titles
...
- repo-aware deck-level workflow operations beyond title promotion and metadata-backed structure planning
